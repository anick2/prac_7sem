...
void SendMinEnergy(std::map<pid_t, Schedule> &solutions, Schedule sol, int fl) {
    ...
    for (auto &x : solutions) {
        int sockfd2 = CreateSocket("BestSolution");
...
Schedule Collect(int NumProcesses, const int &n, const int &m, const std::vector<int> &duration) {

    int sockfd2 = CreateSocket(SADDRESS);
    listen(sockfd2, NumProcesses);
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (solutions.size() == NumProcesses) {
                close(client_socket_fd);
                break;
            }
            close(client_socket_fd);
        ...
        std::cout << currSolution.getEnergy() << std::endl;
        ...
        if (K_out > MAX_ITER_OUT) {
            SendMinEnergy(solutions, minSolution, 2);
            close(sockfd2);
            break;
        } else {
            SendMinEnergy(solutions, minSolution, 1);
        }
    }
    wait(NULL);
    return minSolution;
...
CreateProcceses(const int &NumProcesses, const int &n, const int &m, const std::vector<int> &duration, T &temp, S &sol,
    ...
    if (!collect) {
        ...
        solution = Collect(NumProcesses, n, m, duration);
        for(auto& pid : pids){
            kill(pid, SIGKILL);
        }
        return solution;
...
int main(int argc, char **argv) {
    ...
    while(fin >> line){
        ...
        BoltzmannLaw temperature(intemp);
        ...
        if (numProcesses > 1){
            ...
            SimulatedAnnealing<BoltzmannLaw, Schedule, Mutation> p(temperature, schedule, mutation);
            auto begin = std::chrono::steady_clock::now();
            p.Annealing();
            auto end = std::chrono::steady_clock::now();
            solution = p.GetState();
